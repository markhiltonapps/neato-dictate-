import { NEATODICTATE_API_URL } from "../config/constants.js";
...
async function create(note: NoteInput): Promise<CloudNote> {
  const res = await fetch(`${NEATODICTATE_API_URL}/api/notes/create`, {
    method: "POST",
...
async function batchCreate(
  ...
  const res = await fetch(`${NEATODICTATE_API_URL}/api/notes/batch-create`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
...
async function update(id: string, updates: Partial<NoteInput>): Promise<CloudNote> {
  const res = await fetch(`${NEATODICTATE_API_URL}/api/notes/update`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
...
async function deleteNote(id: string): Promise<void> {
  const res = await fetch(`${NEATODICTATE_API_URL}/api/notes/delete`, {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
...
async function list(limit?: number, before?: string): Promise<{ notes: CloudNote[] }> {
  ...
  const res = await fetch(`${NEATODICTATE_API_URL}/api/notes/list${query ? `?${query}` : ""}`, {
    credentials: "include",
  });
  if (!res.ok) throw new Error(await res.text());
...
async function deleteAll(): Promise<{ deleted: number; errors: number }> {
  ...
  try {
    const res = await fetch(`${NEATODICTATE_API_URL}/api/notes/delete-all`, {
      method: "DELETE",
      credentials: "include",
    });
    if (res.ok) {
...
async function search(query: string, limit?: number): Promise<{ notes: SearchResult[] }> {
  const res = await fetch(`${NEATODICTATE_API_URL}/api/notes/search`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
