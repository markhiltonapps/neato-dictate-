...
function buildMultipartBody(fileBuffer, fileName, contentType, fields = {}) {
  const boundary = `----NeatoDictate${Date.now()}`;
  const parts = [];
...
class IPCHandlers {
  ...
  setupHandlers() {
    ...
    ipcMain.handle("cleanup-app", async (event) => {
      ...
      try {
        const whisperDir = path.join(os.homedir(), ".cache", "neato-dictate", "whisper-models");
            ...
            if (cookieHeader) {
              ...
              if (apiUrl) {
                ...
                if (data.statusCode === 200 && data.data?.text) {
                  result = { text: data.data.text, source: "neato-dictate", model: "cloud" };
                }
              }
            }
          }
    ...
    ipcMain.handle("get-referral-stats", async (event) => {
      try {
        const apiUrl = getApiUrl();
        if (!apiUrl) {
          throw new Error("Neato Dictate API URL not configured");
        }
    ...
    ipcMain.handle("set-debug-logging", async (event, enabled) => {
      try {
        ...
        const logLevelIndex = lines.findIndex((line) =>
          line.trim().startsWith("NEATODICTATE_LOG_LEVEL=")
        );

        if (enabled) {
          // Set to debug
          if (logLevelIndex !== -1) {
            lines[logLevelIndex] = "NEATODICTATE_LOG_LEVEL=debug";
          } else {
            // Add new line
            ...
            lines.push("NEATODICTATE_LOG_LEVEL=debug");
          }
        } else {
          // Remove or set to info
          if (logLevelIndex !== -1) {
            lines[logLevelIndex] = "NEATODICTATE_LOG_LEVEL=info";
          }
        }
        ...
        process.env.NEATODICTATE_LOG_LEVEL = enabled ? "debug" : "info";
