...
export default function EmailVerificationStep({ email, onVerified }: EmailVerificationStepProps) {
  ...
  useEffect(() => {
    if (!NEATODICTATE_API_URL || verified) return;

    const url = `${NEATODICTATE_API_URL}/api/auth/verification-status?email=${encodeURIComponent(email)}`;
  ...
  const handleResend = useCallback(async () => {
    if (resendCooldown > 0 || isResending || !NEATODICTATE_API_URL) return;
    setIsResending(true);
    setError(null);
    try {
      const res = await fetch(`${NEATODICTATE_API_URL}/api/auth/send-verification-email`, {
        method: "POST",
        credentials: "include",
      });
      if (res.ok) {
