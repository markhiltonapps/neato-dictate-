...
```bash
# macOS
/Applications/Neato Dictate.app/Contents/MacOS/Neato Dictate --log-level=debug
...
**Env file locations:**
- macOS: `~/Library/Application Support/Neato Dictate/.env`
- Windows: `%APPDATA%\Neato Dictate\.env`
- Linux: `~/.config/Neato Dictate/.env`

## Log File Locations

- **macOS**: `~/Library/Application Support/Neato Dictate/logs/debug-*.log`
- **Windows**: `%APPDATA%\Neato Dictate\logs\debug-*.log`
- **Linux**: `~/.config/Neato Dictate/logs/debug-*.log`
...
- Remove `NEATODICTATE_LOG_LEVEL` from `.env`
