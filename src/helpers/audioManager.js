...
`;
  ...
  async processAudio(audioBlob, metadata = {}) {
    ...
    try {
      ...
      const isNeatoDictateCloudMode = !useLocalWhisper && cloudTranscriptionMode === "neato-dictate";
      const useCloud = isNeatoDictateCloudMode && isSignedIn;
      logger.debug(
      ...
      } else if (isNeatoDictateCloudMode) {
        if (!isSignedIn) {
          const err = new Error(
            "Neato Dictate Cloud requires sign-in. Please sign in again or switch to BYOK mode."
          );
          err.code = "AUTH_REQUIRED";
          throw err;
        }
        activeModel = "neato-dictate-cloud";
        result = await this.processWithNeatoDictateCloud(audioBlob, metadata);
      } else {
      ...
      if (result?.source === "neato-dictate") {
  ...
  async processWithNeatoDictateCloud(audioBlob, metadata = {}) {
    if (!navigator.onLine) {
    ...
    const reasoningMode = settings.cloudReasoningMode || "neato-dictate";
    if (settings.useReasoningModel && !this.skipReasoning && reasoningMode === "neato-dictate") {
      opts.sendLogs = "false";
    }
    ...
    if (settings.useReasoningModel && processedText && !this.skipReasoning) {
      ...
      const cloudReasoningMode = settings.cloudReasoningMode || "neato-dictate";

      if (cloudReasoningMode === "neato-dictate") {
        try {
          ...
          logger.warn("Cloud reasoning failed — using raw transcription", { error: reasonErr.message }, "transcription");
        }
      } else {
  ...
  shouldUseStreaming(isSignedInOverride) {
    ...
    if (REALTIME_MODELS.has(s.cloudTranscriptionModel)) {
      ...
      if (s.cloudTranscriptionMode === "neato-dictate") return !!(isSignedInOverride ?? s.isSignedIn);
      return false;
    }

    if (s.cloudTranscriptionMode !== "neato-dictate" || !(isSignedInOverride ?? s.isSignedIn)) {
      return false;
    }
      ...
      const [, wsResult] = await Promise.all([
        ...
        withSessionRefresh(async () => {
          ...
          const res = await provider.warmup({
            ...
            mode: cloudTranscriptionMode === "byok" ? "byok" : "neato-dictate",
          });
  ...
  async startStreamingRecording() {
    try {
      ...
      if (error.name === "NotAllowedError" || error.name === "PermissionDeniedError") {
        ...
        errorDescription =
          "Your Neato Dictate Cloud session is unavailable. Please sign in again from Settings.";
      }
  ...
  async stopStreamingRecording() {
    ...
    if (stSettings.useReasoningModel && finalText && !this.skipReasoning) {
      const reasoningStart = performance.now();
      const agentName = localStorage.getItem("agentName") || null;
      const cloudReasoningMode = stSettings.cloudReasoningMode || "neato-dictate";
    ...
    if (!finalText && durationSeconds > 2 && fallbackBlob?.size > 0) {
      ...
      try {
        const batchResult = await this.processWithNeatoDictateCloud(fallbackBlob, {
          durationSeconds,
        });
        if (batchResult?.text) {
