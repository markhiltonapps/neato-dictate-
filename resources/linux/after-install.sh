#!/bin/bash
# Post-install script for Neato Dictate (deb/rpm)
...
CHROME_SANDBOX=$(dpkg -L neato-dictate 2>/dev/null | grep chrome-sandbox || echo "")
if [ -z "$CHROME_SANDBOX" ]; then
  # Fallback: conventional electron-builder install path
  CHROME_SANDBOX="/opt/Neato Dictate/chrome-sandbox"
fi
