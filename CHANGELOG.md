...
- **System Audio Permission Clarity**: Renamed "Screen Recording" to "System Audio" across all permission prompts, onboarding, and settings — makes it clear that Neato Dictate captures other participants' audio, not your screen
...
- **Default Agent Name**: Set default agent name to Neato Dictate
...
- **Batch Transcription Fallback**: If streaming produces no text, automatically falls back to batch transcription via Neato Dictate Cloud
...
- **Neato Dictate Cloud**: Cloud-native transcription service — sign in and transcribe without managing API keys
...
- **Transcription Mode Architecture**: Unified mode selection across Neato Dictate Cloud, Bring Your Own Key (BYOK), and Local
  - Signed-in users default to Neato Dictate Cloud
...
- Button to fully quit Neato Dictate processes from the application
...
- Fixed Neato Dictate documentation paths
...
- Local Whisper now finds Python on Windows more reliably by scanning typical install paths, honoring `NEATODICTATE_PYTHON`, and surfacing actionable ENOENT guidance.
...
- **Menu Labelling**: macOS application menu items now display the correct Neato Dictate casing instead of "neato-dictate".
...
- Initial release of Neato Dictate (formerly OpenWispr)
