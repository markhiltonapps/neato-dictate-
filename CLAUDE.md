# Neato Dictate Technical Reference for AI Assistants

This document provides comprehensive technical details about the Neato Dictate project architecture for AI assistants working on the codebase.

## Project Overview

Neato Dictate is an Electron-based desktop dictation application that uses whisper.cpp for speech-to-text transcription. It supports both local (privacy-focused) and cloud (OpenAI API) processing modes.
...
- **hyprlandShortcut.js**: Hyprland Wayland global shortcut integration
  - Uses D-Bus service to receive hotkey toggle commands (same `com.neatoventures.neatodictate.App` service)
...
- **whisper.js**: Native binary wrapper for local transcription
  ...
  - Models stored in `~/.cache/neato-dictate/whisper-models/`
...
- **parakeet.js**: Model management for NVIDIA Parakeet ASR models
  ...
  - Models stored in `~/.cache/neato-dictate/parakeet-models/`
...
Enable with `--log-level=debug` or `NEATODICTATE_LOG_LEVEL=debug` (can be set in `.env`):
...
On GNOME Wayland, Electron's `globalShortcut` API doesn't work due to Wayland's security model. Neato Dictate uses native GNOME shortcuts:
...
3. `gnomeShortcut.js` creates D-Bus service at `com.neatoventures.neatodictate.App`
...
**Key Constants**:
- D-Bus service: `com.neatoventures.neatodictate.App`
- D-Bus path: `/com/neatoventures/NeatoDictate`
- gsettings path: `/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/neato-dictate/`
...
On Hyprland (wlroots Wayland compositor), Electron's `globalShortcut` API and the `GlobalShortcutsPortal` feature don't work reliably. Neato Dictate uses native Hyprland keybindings:
...
3. `hyprlandShortcut.js` creates D-Bus service at `com.neatoventures.neatodictate.App` (same as GNOME)
...
4. Do NOT translate: brand names (Neato Dictate, Pro), technical terms (Markdown, Signal ID), format names (MP3, WAV), AI system prompts
