...
export default function UploadAudioView({ onNoteCreated, onOpenSettings }: UploadAudioViewProps) {
  ...
  const isNeatoDictateCloud =
    isSignedIn && cloudTranscriptionMode === "neato-dictate" && !useLocalWhisper;
  ...
  // Mode detection
  const isByok = !useLocalWhisper && !isNeatoDictateCloud;
  ...
  if (file) {
    if (useLocalWhisper) {
      ...
      // Cloud (Neato Dictate) — user is always signed in here
  ...
  const getActiveModelLabel = (): string => {
    if (isNeatoDictateCloud) return t("notes.upload.neato-dictateCloud");
    if (useLocalWhisper) {
      if (localTranscriptionProvider === "nvidia")
  ...
  const handleTranscribe = async () => {
    ...
    const useChunkProgress = isNeatoDictateCloud && isLargeFile;
  ...
  const getTranscribingLabel = (): string => {
    if (isNeatoDictateCloud) return t("notes.upload.transcribingCloud");
  ...
  const modeSelector = isSignedIn ? (
    <div className="flex items-center rounded-md border border-foreground/6 dark:border-white/6 bg-surface-1/30 dark:bg-white/[0.02] p-0.5 mb-3">
      ...
      >
        <Cloud size={11} />
        {t("notes.upload.neato-dictateCloud")}
      </button>
      <button
        ...
        <div className="max-w-[320px] mx-auto">
          ...
          {state === "selected" && file && (
            <SelectedView
              ...
              isNeatoDictateCloud={isNeatoDictateCloud}
...
function SelectedView({
  ...
  return (
    <div style={{ animation: "float-up 0.3s ease-out" }}>
      ...
      {isLargeFile && !requiresUpgrade && !fileTooLarge && isNeatoDictateCloud && (
