...
const COMPONENT_NAME = "neato-dictate";
...
class KDEShortcutManager {
  ...
  async _listenForComponent() {
    ...
    try {
      ...
      iface.on("globalShortcutPressed", (componentUnique, shortcutUnique, timestamp) => {
        ...
        // name ("Neato Dictate") not the actionUnique ("dictation"). The fallback
  ...
  _findCallbackByFriendlyName(name) {
    ...
    for (const slotName of this.registeredSlots) {
      friendlyToSlot[`Neato Dictate ${slotName}`] = slotName;
      friendlyToSlot[`Neato Dictate`] = "dictation"; // legacy compat
    }
  ...
  async registerKeybinding(electronHotkey, slotName = "dictation", callback) {
    ...
    const actionId = [COMPONENT_NAME, "Neato Dictate", slotName, `Neato Dictate ${slotName}`];
