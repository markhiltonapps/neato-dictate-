...
static int paste_via_portal(int use_shift, const char *restore_token)
{
    ...
    g_variant_builder_add(&opts, "{sv}", "session_handle_token",
                          g_variant_new_string("neato-dictate"));
...
static int paste_via_uinput(int use_shift) {
    ...
    snprintf(usetup.name, UINPUT_MAX_NAME_SIZE, "neato-dictate-paste");
...
static int send_media_play_pause(void) {
    ...
    if (fd >= 0) {
        if (ioctl(fd, UI_SET_EVBIT, EV_KEY) >= 0 &&
            ...
            usetup.id.vendor  = 0x1234;
            usetup.id.product = 0x5678;
            snprintf(usetup.name, UINPUT_MAX_NAME_SIZE, "neato-dictate-media");
