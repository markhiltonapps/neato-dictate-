...
Neato Dictate supports local speech-to-text processing using whisper.cpp. This keeps your audio completely private—nothing leaves your device.
...
Neato Dictate uses whisper.cpp, a high-performance C++ implementation of OpenAI's Whisper model:

1. whisper.cpp binary is bundled with the app (or uses system installation as fallback)
2. GGML models are downloaded on first use to `~/.cache/neato-dictate/whisper-models/`
...
If you're running Neato Dictate locally from a git checkout (not a packaged app), download the whisper.cpp binary for your current platform:
...
|-------------------|----------------------------------------------|--------------------------------------|---------------------------------|
| Models            | `~/.cache/neato-dictate/whisper-models/`        | `%USERPROFILE%\.cache\neato-dictate\whisper-models\` | `~/.cache/neato-dictate/whisper-models/` |
