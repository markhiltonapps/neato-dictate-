# Neato Dictate Complete Cleanup Instructions
...
```bash
cd /path/to/neato-dictate
bash scripts/complete-uninstall.sh
```

This script will:
- Stop all Neato Dictate processes
...
### 1. Stop all Neato Dictate processes

```bash
# Kill production app
pkill -f "Neato Dictate"
...
```bash
rm -rf /Applications/Neato Dictate.app
```
...
```bash
rm -rf "$HOME/Library/Application Support/Neato Dictate"
rm -rf "$HOME/Library/Application Support/neato-dictate"
```

**Preferences** (system-level settings):
```bash
rm -rf "$HOME/Library/Preferences/com.neatoventures.neatodictate.app.plist"
rm -rf "$HOME/Library/Preferences/com.electron.neato-dictate.plist"
```

**Caches**:
```bash
rm -rf "$HOME/Library/Caches/Neato Dictate"
rm -rf "$HOME/Library/Caches/neato-dictate"
```

**Logs**:
```bash
rm -rf "$HOME/Library/Logs/Neato Dictate"
rm -rf "$HOME/Library/Logs/neato-dictate"
```

**Saved Application State**:
```bash
rm -rf "$HOME/Library/Saved Application State/com.neatoventures.neatodictate.app.savedState"
```
...
find /tmp -name "neato-dictate_*" -delete
```
...
All Neato Dictate data is stored in these locations:

| Type | Location |
|------|----------|
| **Databases** | `~/Library/Application Support/Neato Dictate/transcriptions.db` |
| **Dev Database** | `~/Library/Application Support/Neato Dictate/transcriptions-dev.db` |
| **Settings** | Browser localStorage (in Electron's userData) |
| **API Keys** | `.env` file in project root (dev) |
| **Logs** | `~/Library/Application Support/Neato Dictate/logs/` |
| **Debug Logs** | `~/Library/Logs/Neato Dictate/` |
| **Whisper Models** | `~/.cache/whisper/` |
| **Preferences** | `~/Library/Preferences/com.neatoventures.neatodictate.app.plist` |
| **Caches** | `~/Library/Caches/Neato Dictate/` |
...
5. Check logs in: `~/Library/Application Support/Neato Dictate/logs/`

Report issues with this information at: [GitHub Issues](https://github.com/your-repo/neato-dictate/issues)
