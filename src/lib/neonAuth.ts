...
import { NEATODICTATE_API_URL } from "../config/constants";
...
const LAST_SIGN_IN_STORAGE_KEY = "neatoDictate:lastSignInTime";
...
export async function deleteAccount(): Promise<{ error?: Error }> {
  if (!NEATODICTATE_API_URL) {
    return { error: new Error("API not configured") };
  }

  try {
    const res = await fetch(`${NEATODICTATE_API_URL}/api/auth/delete-account`, {
      method: "DELETE",
      credentials: "include",
    });
...
function getElectronOAuthCallbackURL(): string {
  const configuredUrl = (import.meta.env.VITE_NEATODICTATE_OAUTH_CALLBACK_URL || "").trim();
...
export async function requestPasswordReset(email: string): Promise<{ error?: Error }> {
  ...
  try {
    if (NEATODICTATE_API_URL) {
      const res = await fetch(`${NEATODICTATE_API_URL}/api/auth/forgot-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
