<p align="center">
  <img src="src/assets/logo.svg" alt="Neato Dictate" width="120" />
</p>

<h1 align="center">Neato Dictate</h1>

<p align="center">
  <a href="https://github.com/NeatoVentures/neato-dictate/blob/main/LICENSE"><img src="https://img.shields.io/github/license/NeatoVentures/neato-dictate?style=flat" alt="License" /></a>
  <img src="https://img.shields.io/badge/platform-macOS%20%7C%20Windows%20%7C%20Linux-lightgrey?style=flat" alt="Platform" />
  <a href="https://github.com/NeatoVentures/neato-dictate/releases/latest"><img src="https://img.shields.io/github/v/release/NeatoVentures/neato-dictate?style=flat&sort=semver" alt="GitHub release" /></a>
  <a href="https://github.com/NeatoVentures/neato-dictate/releases"><img src="https://img.shields.io/github/downloads/NeatoVentures/neato-dictate/total?style=flat&color=blue" alt="Downloads" /></a>
  <a href="https://github.com/NeatoVentures/neato-dictate/stargazers"><img src="https://img.shields.io/github/stars/NeatoVentures/neato-dictate?style=flat" alt="GitHub stars" /></a>
</p>
...
[![Star History Chart](https://api.star-history.com/svg?repos=NeatoVentures/neato-dictate&type=date&legend=top-left)](https://www.star-history.com/#NeatoVentures/neato-dictate&type=date&legend=top-left)
...
- ☁️ **Neato Dictate Cloud**: Sign in and transcribe instantly — no API keys needed, with free and Pro plans
...
1. **Clone the repository**:
   ...
   ```bash
   git clone https://github.com/NeatoVentures/neato-dictate.git
   cd neato-dictate
   ```
...
# The unsigned app will be in: dist/mac-arm64/Neato Dictate.app (macOS)
# or dist/win-unpacked/Neato Dictate.exe (Windows)
# or dist/linux-unpacked/neato-dictate (Linux)
```
...
Neato Dictate ships a native C binary for pasting text on Windows, using the Win32 `SendInput` API. This is the **primary** paste mechanism — `nircmd` and PowerShell are only used as fallbacks if the native binary fails.
...
Neato Dictate now supports multiple Linux package formats for maximum compatibility:
...
# - Neato Dictate-x.x.x-linux-x64.AppImage
# - Neato Dictate-x.x.x-linux-x64.deb
# - Neato Dictate-x.x.x-linux-x64.rpm
# - Neato Dictate-x.x.x-linux-x64.tar.gz
```
...
```bash
# Debian/Ubuntu
sudo apt install ./dist/Neato Dictate-*-linux-x64.deb

# Fedora/RHEL
sudo dnf install ./dist/Neato Dictate-*-linux-x64.rpm

# Universal tar.gz (no root required)
tar -xzf dist/Neato Dictate-*-linux-x64.tar.gz
cd Neato Dictate-*/
./neato-dictate

# Flatpak
flatpak install --user ./dist/Neato Dictate-*-linux-x64.flatpak

# AppImage (existing method)
chmod +x dist/Neato Dictate-*.AppImage
./dist/Neato Dictate-*.AppImage
```

**Native Paste Binary (`linux-fast-paste`)**:

Neato Dictate ships a native C binary for pasting text on Linux, compiled automatically at build time. This is the **primary** paste mechanism — external tools like `xdotool` and `wtype` are only used as fallbacks if the native binary fails.
...
If the native binary isn't available, Neato Dictate falls back to external paste tools in this order:
...
> ℹ️ **Note**: Neato Dictate automatically tries paste methods in this order: native `linux-fast-paste` binary (XTest or uinput) → `wtype` → `ydotool` → `xdotool` (for XWayland apps). If no paste method works, text will still be copied to the clipboard - you'll just need to paste manually with Ctrl+V.
...
On GNOME Wayland, Electron's standard global shortcuts don't work due to Wayland's security model. Neato Dictate automatically uses native GNOME keyboard shortcuts via D-Bus and gsettings:
...
1. **Choose Processing Method**:
   - **Neato Dictate Cloud**: Sign in for instant cloud transcription with free and Pro plans
...
- **In-App**: Use _Settings → General → Local Model Storage → Remove Downloaded Models_ to clear cached Whisper and Parakeet models from `~/.cache/neato-dictate/` (or `%USERPROFILE%\.cache\neato-dictate\` on Windows).
...
- **macOS**: If you uninstall manually, remove `~/Library/Caches` or `~/.cache/neato-dictate/` if desired.
...
- Brand names (e.g., "Neato Dictate", "whisper.cpp")
...
For local processing, Neato Dictate uses OpenAI's Whisper model via whisper.cpp - a high-performance C++ implementation:

1. **Bundled Binary**: whisper.cpp is bundled with the app for all platforms
2. **GGML Models**: Downloads optimized GGML models on first use to `~/.cache/neato-dictate/whisper-models/`
...
**Upgrading from Python-based version**: If you previously used the Python-based Whisper, you'll need to re-download models in GGML format. You can safely delete the old Python environment (`~/.neato-dictate/python/`) and PyTorch models (`~/.cache/whisper/`) to reclaim disk space.

### Local Parakeet Setup (Alternative)

Neato Dictate also supports NVIDIA Parakeet models via sherpa-onnx - a fast alternative to Whisper:

1. **Bundled Binary**: sherpa-onnx is bundled with the app for all platforms
2. **INT8 Quantized Models**: Efficient CPU inference
3. **Models stored in**: `~/.cache/neato-dictate/parakeet-models/`
...
Neato Dictate is designed with privacy and security in mind:
...
2. **Accessibility permissions (macOS)**: Required for automatic text pasting
   ...
   - Add Neato Dictate and enable the checkbox
...
- Check the [Issues](https://github.com/NeatoVentures/neato-dictate/issues) page
...
**Q: Is Neato Dictate really free?**
A: Yes! Neato Dictate is open source and free to use. The free plan includes 2,000 words/week of cloud transcription, and local processing is completely free with no limits. Paid plans start from as little as $8/month.
...
A: Neato Dictate supports 58 languages including English, Spanish, French, German, Chinese, Japanese, and more. Set your preferred language in the .env file or use auto-detect.
...
A: Connect your Google Calendar in Integrations. When a meeting starts (Zoom, Teams, FaceTime), Neato Dictate detects it and offers to record. Audio is transcribed in real-time via OpenAI Realtime API. On macOS, screen recording permission is required to capture meeting audio.
...
**Q: Does Neato Dictate require Input Monitoring on macOS?**
A: No. As of v1.6.0, Neato Dictate uses NSEvent monitors instead of CGEvent taps, eliminating the Input Monitoring permission requirement. Only Microphone and Accessibility permissions are needed (plus Screen Recording for meeting features).

## Project Status

Neato Dictate is actively maintained and ready for production use. Current version: 1.6.6

- ✅ Core dictation with local and cloud processing
- ✅ Cross-platform support (macOS, Windows, Linux)
- ✅ Neato Dictate Cloud with account system, usage tracking, and Stripe billing
