...
export default function AuthenticationStep({
  ...
  useEffect(() => {
    ...
    const initAndComplete = async () => {
      if (NEATODICTATE_API_URL) {
        try {
          const res = await fetch(`${NEATODICTATE_API_URL}/api/auth/init-user`, {
            method: "POST",
  ...
  const handleEmailContinue = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${NEATODICTATE_API_URL}/api/check-user`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      try {
        if (authMode === "sign-up") {
          ...
          if (result.error) {
            ...
            if (NEATODICTATE_API_URL) {
              try {
                await fetch(`${NEATODICTATE_API_URL}/api/auth/init-user`, {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-muted-foreground/80 leading-tight text-center">
        {t("auth.legal.prefix")}{" "}
        <a
          href="https://neatodictate.com/terms"
          target="_blank"
          rel="noopener noreferrer"
        ...
        {t("auth.legal.and")}{" "}
        <a
          href="https://neatodictate.com/privacy"
          target="_blank"
          rel="noopener noreferrer"
          className="text-link underline decoration-link/30 hover:decoration-link/60 transition-colors"
        >
