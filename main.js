...
const DEFAULT_OAUTH_PROTOCOL_BY_CHANNEL = {
  ...
  staging: "neato-dictate-staging",
  production: "neato-dictate",
};
const BASE_WINDOWS_APP_ID = "com.neatoventures.neatodictate";
...
function resolveAppChannel() {
  const rawChannel = (process.env.NEATODICTATE_CHANNEL || process.env.VITE_NEATODICTATE_CHANNEL || "")
    .trim()
    .toLowerCase();
...
process.env.NEATODICTATE_CHANNEL = APP_CHANNEL;
...
function configureChannelUserDataPath() {
  ...
  const isolatedPath = path.join(app.getPath("appData"), `NeatoDictate-${APP_CHANNEL}`);
...
if (process.platform === "linux") {
  app.setDesktopName("neato-dictate.desktop");
}
...
function getOAuthProtocol() {
  const fromEnv = (process.env.VITE_NEATODICTATE_PROTOCOL || process.env.NEATODICTATE_PROTOCOL || "")
    .trim()
    .toLowerCase();
...
function registerNeatoDictateProtocol() {
...
const protocolRegistered = registerNeatoDictateProtocol();
...
if (process.platform === "darwin" && app.getName() !== "Neato Dictate") {
  app.setName("Neato Dictate");
}
...
function parseAuthBridgePort() {
  const raw = (process.env.NEATODICTATE_AUTH_BRIDGE_PORT || "").trim();
...
function startAuthBridgeServer() {
  ...
  authBridgeServer = http.createServer(async (req, res) => {
    ...
    res.end(
      "<html><body><h3>Neato Dictate sign-in complete.</h3><p>You can close this tab.</p></body></html>"
    );
  });
