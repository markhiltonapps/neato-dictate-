...
function TranscriptionSection({
  ...
  const isCloudMode = isSignedIn && cloudTranscriptionMode === "neato-dictate" && !useLocalWhisper;
        ...
        <SettingsPanel>
          <SettingsPanelRow>
            <button
              onClick={() => {
                if (!isCloudMode) {
                  setCloudTranscriptionMode("neato-dictate");
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-medium text-foreground">
                    {t("settingsPage.transcription.neato-dictateCloud")}
                  </span>
                  {isCloudMode && (
                ...
                <p className="text-xs text-muted-foreground/80 mt-0.5">
                  {t("settingsPage.transcription.neato-dictateCloudDescription")}
                </p>
              </div>
              <div
...
function AiModelsSection({
  ...
  const isCloudMode = isSignedIn && cloudReasoningMode === "neato-dictate";
            ...
            <SettingsPanel>
              <SettingsPanelRow>
                <button
                  onClick={() => {
                    if (!isCloudMode) {
                      setCloudReasoningMode("neato-dictate");
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-medium text-foreground">
                        {t("settingsPage.aiModels.neato-dictateCloud")}
                      </span>
                      {isCloudMode && (
                        <span className="text-xs font-medium text-primary bg-primary/10 dark:bg-primary/15 px-1.5 py-px rounded-sm">
                          {t("common.active")}
                    ...
                    <p className="text-xs text-muted-foreground/80 mt-0.5">
                      {t("settingsPage.aiModels.neato-dictateCloudDescription")}
                    </p>
                  </div>
                  <div
                    className={`w-4 h-4 rounded-full border-2 shrink-0 transition-colors ${
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (anyFailed) {
            ...
            window.dispatchEvent(new Event("neato-dictate-models-cleared"));
            showAlertDialog({
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  ...
                  <div className="rounded-md border border-border/50 dark:border-border-subtle/60 bg-card/30 dark:bg-surface-2/30 p-2.5 flex flex-col">
                    ...
                    <Button
                      ...
                      className="mt-2 w-full h-6 text-[10px]"
                      onClick={() =>
                        window.electronAPI?.openExternal?.("https://neatodictate.com/contact-sales")
                      }
                    >
                      <Mail size={10} />
                      ...
                      steps: [
                        ...
                        },
                        {
                          ...
                          desc: t("settingsPage.general.waylandPaste.guide.group.step2Desc", {
                            defaultValue:
                              "Group changes only take effect after a new login session. Log out of your desktop and log back in, then reopen Neato Dictate.",
                          }),
                        },
                      ],
                    },
